...
describe("Shadow DOM", () => {
  it("works", async () => {
    ...
    expect(el.shadowRoot.firstChild.nextSibling.nodeValue).to.equal(
      "using ShadowDOM"
    );
  });
  ...
  it("is optional", async () => {
    ...
    const el = await fixture<HTMLElement>(
      html`<no-shadow-test></no-shadow-test>`
    );
  ...
  it("defaults to not delegating focus from the Shadow DOM", async () => {
    ...
    const el = await fixture<HTMLElement>(
      html`<default-delegates-focus></default-delegates-focus>`
    );
