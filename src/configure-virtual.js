...
class DirectiveContainer extends Container {
  constructor(render, renderer, part) {
    super(render, renderer, part);
    this.virtual = true;
  }
...
function agnosticVirtual(directive, render, renderer) {
  function factory(...args) {
    return part => {
      let cont = partToContainer.get(part);
      if(!cont) {
        cont = new DirectiveContainer(render, renderer, part);
...
const configureVirtual = ({directive, render}) =>
  renderer => agnosticVirtual(directive, render, renderer);
